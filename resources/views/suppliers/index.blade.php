        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600">Manage your suppliers</p>
                </div>
                <a href="{{ route('suppliers.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700">
                    + Add Supplier
                </a>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('suppliers.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Name, contact, or email..." class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm">
                    </div>
                    <div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('suppliers.index') }}" class="px-6 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 font-medium text-sm transition">
                            <i class="fas fa-redo mr-2"></i>Reset
                        </a>
                    </div>
                    <div class="text-sm text-gray-600">
                        Showing <strong>{{ $suppliers->total() }}</strong> suppliers
                    </div>
                </div>
            </form>
        </div>
                            ...
                            @foreach($suppliers as $supplier)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        ...
                                        <a href="{{ route('suppliers.edit', $supplier) }}" class="ml-4 text-yellow-600 hover:text-yellow-900">Edit</a>
